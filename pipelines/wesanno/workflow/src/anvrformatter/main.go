...
func ChangeColName(colNames []string, renameMap map[string]string) []string {
	updatedColNames := make([]string, len(colNames))
	for i, colName := range colNames {
		if newName, ok := renameMap[colName]; ok {
			updatedColNames[i] = newName
		} else {
			updatedColNames[i] = colName
		}
	}
	return updatedColNames
}
...
func RenameDuplicateCols(colNames []string) []string {
	colCountMap := make(map[string]int)
	for i, colName := range colNames {
		count := colCountMap[colName]
		if count > 0 {
			colNames[i] = fmt.Sprintf("%d_%s", count+1, colName)
			colCountMap[colName]++
		} else {
			// If the column name does not exist, add it to the map
			// and set the count to 1 and no need to rename
			colCountMap[colName] = 1
		}
	}
	return colNames
}
...
func RenameFilterProcess(txtPath string, samples []string, renamemap map[string]string) []string {
	...
	for txtScanner.Scan() {
		...
		if lineNum == 0 {
			...
			unDuplicatedColNames := RenameDuplicateCols(colNames)

			// 2nd process: Get the index of "CHROM" and "Func.refGene"
			chromIndex = GetColumnIndex(unDuplicatedColNames, "Chr")
			funcIndex = GetColumnIndex(unDuplicatedColNames, "Func.refGene")
			...
			var newCols []string = GenerateNewColNames(unDuplicatedColNames, samples)
