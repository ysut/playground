process FIND_XAM {
    // publishDir "${params.out_root}/${family_id}/tmp", mode: 'symlink'

    input:
    tuple val(family_id), val(sample_id)

    output:
    tuple val(family_id), val(sample_id), path("${sample_id}.*am"), path("${sample_id}.*ai")

    script:
    """
    mkdir -p "${params.out_root}/${family_id}/xams" &&
    
    find "${params.out_root}/" -name "*${sample_id}*.{b,cr}am" \\
        -exec ln -s {} "${params.out_root}/${family_id}/xams" \\; &&
    find "${params.out_root}/tmp" -name "*${sample_id}*.{b,cr}ai" \\
        -exec ln -s {} "${params.out_root}/${family_id}/xams" \\;
    """
}
